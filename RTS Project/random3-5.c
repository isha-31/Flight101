...
void avionicsTaskHandler(void *pvParameters) {
  Task *task = (Task *)pvParameters;
  BaseType_t higherPriorityTaskWoken = pdFALSE;       // Flag to indicate if a higher priority task was woken
  UBaseType_t taskPriority = uxTaskPriorityGet(NULL); // Get default task priority

  // Check criticality level and adjust task priority accordingly
  switch (task->criticality) {
  case 'A':                                  // Highest criticality
    taskPriority = configMAX_PRIORITIES - 1; // Set task priority to highest
    break;
  case 'B':                                  // High criticality
    taskPriority = configMAX_PRIORITIES - 2; // Set task priority one level below highest
    break;
  case 'C':                                  // Medium criticality
    taskPriority = configMAX_PRIORITIES / 2; // Set task priority to middle
    break;
  case 'D':                                  // Low criticality
    taskPriority = configMAX_PRIORITIES / 4; // Set task priority one level above lowest
    break;
  case 'E':           // Lowest criticality
    taskPriority = 1; // Set task priority to lowest
    break;
  default:
    // Handle invalid criticality level
    break;
  }

  vTaskPrioritySet(NULL, taskPriority); // Set task priority based on criticality
  ...
  // If a higher priority task was woken during execution, yield to it
  if (higherPriorityTaskWoken == pdTRUE) {
    taskYIELD(); // Trigger task switch to the higher priority task
  }
